...
const time = {
  ...
  addDay(data) {
    const { expired, goalTerm } = data;
    
    const tmp = moment(expired).add(goalTerm, 'days')
    const result = this.changeFormat(tmp);

    return result;
  },

  diffDay(future, past) {
    const newFuture = moment.tz(future, 'Asia/Seoul');
    const newPast = moment.tz(past, 'Asia/Seoul');
    return newFuture.diff(newPast, 'days');
  }
